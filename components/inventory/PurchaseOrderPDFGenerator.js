// components/inventory/PurchaseOrderPDFGenerator.js
"use client";
...
// Styles - Maintaining black/white professional theme
const styles = StyleSheet.create({
    ...
    // Header styles - Matching existing format
    headerContainer: {
        marginBottom: 25,
        borderBottomWidth: 2,
    ...
    // Table styles - Adjusted to prevent overlapping
    tableTitle: {
        color: '#222',
        fontSize: 12,
        fontWeight: 'bold',
    ...
    // Footer - Matching existing format
    footer: {
        marginTop: 30,
        paddingTop: 20,
        fontSize: 9,
...
const POHeader = ({ purchaseOrder, companyLogo }) => (
    <View style={styles.headerContainer}>
        <View style={styles.headerRow}>
            ...
            <View style={styles.poBadge}>
                ...
                <Text style={styles.poNumber}>
                    <Text style={{ fontWeight: 'bold' }}>No: </Text>{purchaseOrder.poNumber || 'N/A'}
                </Text>
            </View>
        </View>
        <View style={styles.headerDetails}>
            ...
            <View style={{ textAlign: 'right' }}>
                <Text><Text style={{ fontWeight: 'bold' }}>Date:</Text> {formatDate(purchaseOrder.orderDate)}</Text>
                <Text><Text style={{ fontWeight: 'bold' }}>Delivery By:</Text> {formatDate(purchaseOrder.expectedDeliveryDate)}</Text>
            </View>
        </View>
    </View>
);

// Component for vendor and order details
const PODetailsSection = ({ purchaseOrder, vendor }) => {
            ...
            <View style={styles.detailsRow}>
                ...
                <View style={styles.detailBox}>
                    ...
                    <View style={styles.detailContent}>
                        <Text style={styles.vendorName}>{vendor?.companyName || purchaseOrder.vendorName || 'Vendor Name'}</Text>
                        {vendor?.contactPerson && (
                            <Text style={styles.vendorText}>Attn: {vendor.contactPerson}</Text>
                        )}
                        {vendor?.address && (
                            <Text style={styles.vendorText}>{formatAddress(vendor.address)}</Text>
                        )}
                        {vendor?.phone && (
                        ...
                        {vendor?.gstNumber && (
                            <Text style={styles.vendorText}>GSTIN: {vendor.gstNumber}</Text>
                        )}
                    </View>
                </View>
                ...
                <View style={styles.detailBox}>
                    ...
                    <View style={styles.detailContent}>
                        <View style={styles.detailRow}>
                            ...
                            <Text style={styles.detailValue}>{formatDate(purchaseOrder.orderDate)}</Text>
                        </View>
                        <View style={styles.detailRow}>
                            <Text style={styles.detailLabel}>Delivery Date:</Text>
                            <Text style={styles.detailValue}>{formatDate(purchaseOrder.expectedDeliveryDate)}</Text>
                        </View>
                        <View style={styles.detailRow}>
                            <Text style={styles.detailLabel}>Payment Terms:</Text>
                            <Text style={styles.detailValue}>{purchaseOrder.paymentTerms || 'Net 30 days'}</Text>
                        </View>
                        <View style={styles.detailRow}>
                            <Text style={styles.detailLabel}>Status:</Text>
                            <Text style={styles.detailValue}>{purchaseOrder.status || 'DRAFT'}</Text>
                        </View>
                    </View>
                </View>
            </View>
...
// Also, fix any other places where objects might be rendered as text
// In the POInfoSection component, make sure all values are strings:
const POInfoSection = ({ purchaseOrder }) => (
    <View style={styles.poInfoContainer}>
        <Text style={styles.poInfoTitle}>SHIPPING INFORMATION</Text>
        <View style={styles.poInfoContent}>
            <View style={styles.poInfoRow}>
                <Text style={styles.poInfoLabel}>Ship To:</Text>
                <Text style={styles.poInfoValue}>Grav Clothing, Mayfair Lagoon Campus, Bhubaneswar</Text>
            </View>
            <View style={styles.poInfoRow}>
                <Text style={styles.poInfoLabel}>Contact Person:</Text>
                <Text style={styles.poInfoValue}>Purchasing Manager</Text>
            </View>
            <View style={styles.poInfoRow}>
                <Text style={styles.poInfoLabel}>Phone:</Text>
                <Text style={styles.poInfoValue}>+91-XXXXXXXXXX</Text>
            </View>
            <View style={{ ...styles.poInfoRow, borderBottomWidth: 0, paddingBottom: 0 }}>
                <Text style={styles.poInfoLabel}>Delivery Instructions:</Text>
                <Text style={styles.poInfoValue}>Please deliver during business hours (9 AM - 6 PM)</Text>
            </View>
        </View>
    </View>
);

// Component for items table - FIXED to prevent overlapping
const POItemsTable = ({ items }) => {
    // Filter out items with zero or negative prices
    const validItems = items.filter(item =>
        getValue(item.unitPrice, 0) > 0 && getValue(item.quantity, 0) > 0
    );

    if (validItems.length === 0) {
        return (
            <View style={{ padding: 20, alignItems: 'center', backgroundColor: '#f9f9f9', borderRadius: 4 }}>
                <Text style={{ color: '#777', fontSize: 11, fontStyle: 'italic' }}>
    ...
    return (
        <View wrap={false}>
            <Text style={styles.tableTitle}>ITEMS ORDERED</Text>
            <View style={styles.table}>
                ...
                <View style={styles.tableHeader}>
                    ...
                    <View style={[styles.tableCellCenter, styles.unitCell]}>
                ...
                {validItems.map((item, index) => {
                    ...
                    const total = getValue(item.totalPrice, quantity * unitPrice);
                    ...
                    return (
                        <View key={index} style={[
                            ...
                            <View style={[styles.tableCellCenter, styles.srNoCell]}>
                                <Text style={{ fontWeight: 'bold', fontSize: 9 }}>{index + 1}</Text>
                            </View>
                            <View style={[styles.tableCell, styles.descCell]}>
                                <Text style={styles.itemName}>{item.itemName || 'Raw Material'}</Text>
                                {item.description && (
                                    <Text style={styles.itemDescription}>{item.description}</Text>
                                )}
                                {item.rawItem?.category && (
                                    <Text style={styles.itemCategory}>Category: {item.rawItem.category}</Text>
                                )}
                            </View>
                            
                            <View style={[styles.tableCellCenter, styles.qtyCell]}>
                                <Text style={{ fontWeight: 'bold', fontSize: 9 }}>{formatNumber(quantity)}</Text>
                            </View>
                            <View style={[styles.tableCellCenter, styles.unitCell]}>
                                <Text style={{ fontSize: 9, color: '#666' }}>{item.unit || 'PCS'}</Text>
                            </View>
                            <View style={[styles.tableCellRight, styles.priceCell]}>
                                <Text style={{ fontSize: 9 }}>{formatIndianRupees(unitPrice)}</Text>
                            </View>
                            <View style={[styles.tableCellRight, styles.totalCell]}>
                                <Text style={{ fontWeight: 'bold', color: '#111', fontSize: 10 }}>
...
const POSummarySection = ({ purchaseOrder }) => {
    const subtotal = getValue(purchaseOrder.subtotal, 0);
    const taxRate = getValue(purchaseOrder.taxRate, 0);
    const taxAmount = getValue(purchaseOrder.taxAmount, 0);
    const shippingCharges = getValue(purchaseOrder.shippingCharges, 0);
    const discount = getValue(purchaseOrder.discount, 0);
    const totalAmount = getValue(purchaseOrder.totalAmount, 0);
        ...
        <View style={styles.summaryContainer}>
            ...
            <View style={styles.summaryContent}>
                ...
                <View style={styles.summaryRow}>
                    ...
                    <Text style={styles.summaryValue}>{formatIndianRupees(subtotal)}</Text>
                </View>
                ...
                {discount > 0 && (
                    <View style={styles.summaryRow}>
                        ...
                        <Text style={styles.discountValue}>- {formatIndianRupees(discount)}</Text>
                    </View>
                )}
                ...
                {taxAmount > 0 && (
                    <View style={styles.summaryRow}>
                        ...
                        <Text style={styles.taxValue}>+ {formatIndianRupees(taxAmount)}</Text>
                    </View>
                )}
                ...
                {shippingCharges > 0 && (
                    <View style={styles.summaryRow}>
                        ...
                        <Text style={styles.summaryValue}>+ {formatIndianRupees(shippingCharges)}</Text>
                    </View>
                )}
                ...
                <View style={[styles.summaryRow, styles.totalRow]}>
                    ...
                    <Text style={styles.totalValue}>{formatIndianRupees(totalAmount)}</Text>
                </View>
            </View>
        </View>
    );
...
const POTermsAndConditions = ({ purchaseOrder }) => (
    <View style={styles.termsContainer}>
        <Text style={styles.detailTitle}>TERMS AND CONDITIONS</Text>
        <View style={styles.termsContent}>
            <Text style={{ color: '#555', lineHeight: 1.5 }}>
                {purchaseOrder.termsConditions ? (
                    purchaseOrder.termsConditions.split('\n').map((line, i) => (
                        <Text key={i}>
                            {line}
                            {i < purchaseOrder.termsConditions.split('\n').length - 1 && '\n'}
                        </Text>
                    ))
                ) : (
                    `1. This purchase order is valid until the delivery date mentioned above.\n` +
                    `2. All goods must be delivered in good condition and as per specifications.\n` +
                    `3. Invoices must reference this PO number for payment processing.\n` +
                    `4. Payment terms: ${purchaseOrder.paymentTerms || 'Net 30 days from invoice date'}.\n` +
                    `5. Early delivery requires prior approval.\n` +
                    `6. Non-conforming goods will be rejected at vendor's cost.\n` +
                    `7. All taxes applicable as per current laws.\n` +
...
// Component for notes
const PONotesSection = ({ purchaseOrder }) => {
    ...
    return (
        <View style={styles.notesContainer}>
            ...
            <View style={styles.detailContent}>
                <Text style={{ color: '#555', fontSize: 10, lineHeight: 1.5 }}>
                    {purchaseOrder.notes}
                </Text>
            </View>
        </View>
...
// Component for footer with signatures - Matching existing format
const POFooter = ({ stampSignature }) => (
    <View style={styles.footer} wrap={false}>
        <View style={styles.footerRow}>
            ...
            <View style={styles.signatureSection}>
                <View style={styles.signatureContainer}>
                    ...
                    <Text style={styles.signatureLabel}>FOR VENDOR</Text>
                    <Text style={styles.signatureSubLabel}>Signature with Company Stamp</Text>
                    <Text style={styles.signatureSubLabel}>Name: _______________________</Text>
                    <Text style={styles.signatureSubLabel}>Date: _______________________</Text>
                </View>
            </View>
        ...
        <View style={styles.footerDivider}>
            <Text style={styles.footerText}>This is a computer-generated purchase order.</Text>
...
const PurchaseOrderPDFDocument = ({
    ...
    const validItems = purchaseOrder.items?.filter(item =>
        item && getValue(item.quantity, 0) > 0 && getValue(item.unitPrice, 0) > 0
    ) || [];
        ...
        <Document>
            <Page size="A4" style={styles.page}>
                <View style={styles.container}>
                    {/* Header */}
                    <POHeader purchaseOrder={purchaseOrder} companyLogo={companyLogo} />

                    {/* Vendor and Order Details */}
                    <PODetailsSection purchaseOrder={purchaseOrder} vendor={vendor} />

                    {/* Shipping Information */}
                    <POInfoSection purchaseOrder={purchaseOrder} />

                    {/* Items Table */}
                    <POItemsTable items={validItems} />

                    {/* Price Summary */}
                    <POSummarySection purchaseOrder={purchaseOrder} />

                    {/* Notes (if any) */}
                    <PONotesSection purchaseOrder={purchaseOrder} />

                    {/* Terms and Conditions */}
                    <POTermsAndConditions purchaseOrder={purchaseOrder} />

                    {/* Footer */}
                    <POFooter stampSignature={stampSignature} />
                </View>
            </Page>
        </Document>
    );
...
export default function PurchaseOrderPDFDownload({
    purchaseOrder,
    vendor,
    ...
    if (!purchaseOrder.poNumber) {
        return (
            <button
                onClick={() => alert("Incomplete purchase order data. PO number is required.")}
    ...
    return (
        <PDFDownloadLink
            document={
                <PurchaseOrderPDFDocument
                    ...
                    stampSignature={customerSignature}
                />
            }
            fileName={`PO_${purchaseOrder.poNumber}_${formatDate(purchaseOrder.orderDate).replace(/ /g, '_')}.pdf`}
        ...
        >
            {({ loading }) => (
                <>
                    <Download className="w-4 h-4" />
                    {loading ? 'Generating...' : 'Download PO PDF'}
                </>
            )}
        </PDFDownloadLink>
    );
